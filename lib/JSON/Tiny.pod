...
Comparing JSON::PP to JSON::Tiny:


=over 4

=item * L<JSON> is highly configurable.  L<JSON::Tiny> provides sane defaults,
and a simple zero-configuration API.
...
dependencies.  L<JSON> is backward compatible to Perl 5.6.x, while 
C<JSON::Tiny> is backward compatible to 5.10.x.

=item * Simple Design: L<JSON> is 2254 lines of code spread across six modules 
and five files with a distribution tarball size of 84KB. L<JSON::Tiny> is 351 
lines of code in a single module, and a single file.  It can even be embedded 
within existing code.  The tarball size is 17KB.

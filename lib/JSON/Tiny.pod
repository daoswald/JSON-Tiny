...
under 350 lines of code and core-only dependencies.
...
L<Mojo::JSON> was chosen as a start point because it is robust, minimal, and
well tested. Mojo::JSON's tests were able to be adapted as easily as the module
itself to a design free of non-core dependencies.
...
files, and a dist tarball of 84KB. L<JSON::Tiny> has 350 lines of code; a single
module in a single file. It can be easily embedded within existing code. The
tarball is 18KB.
...
This module needs Perl version 5.10 or newer.
...
Also to Randal Schwartz for showing the Los Angeles Perl Mongers (Sept 2012) his
embeddable pure-regexp JSON parser, and explaining it on PerlMonks
(L<http://www.perlmonks.org/?node_id=995856>). He wasn't involved in JSON::Tiny,
but it was the exploration of alternatives to his solution that led to this fork
of Mojolicious's JSON parser.

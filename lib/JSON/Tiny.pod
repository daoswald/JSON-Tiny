...
Key features include L<RFC 4627|http://tools.ietf.org/html/rfc4627> JSON, 
transparent Unicode support, speed, small memory footprint, and a minimal 
code base ideal for bundling or inlining. L<JSON::Tiny> and Mojo::JSON are 
possibly the fastest pure-Perl RFC4627 JSON implementations.
...
B<This function has been deprecated.  RFC7159 specifies that scalars
are now legitimate encode/decode entities, which makes j()'s ability
to decide whether to encode or decode an intractable question.  This
function will be removed in an upcoming release without further
warning.>

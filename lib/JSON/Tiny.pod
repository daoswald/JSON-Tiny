...
The Boolean false and true values returned when JSON is decoded are
...
will have the desired effect.  Using C<local> to prevent the change from
causing errors at a distance is probably a very good idea.  

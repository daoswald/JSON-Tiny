...
=item * L<JSON::PP> is configurable, and comparatively complex. L<JSON::Tiny>
offers sane defaults, and nothing to configure.

=item * Installation: C<cpanm JSON::PP> vs C<cpanm JSON::Tiny>: L<JSON::PP>:
5.2 seconds. L<JSON::Tiny>: 1.9 seconds (including download).
...
These utilities measure memory in different ways, but both show JSON::Tiny is
600-700KB smaller than JSON::PP.
...
adapted directly from L<Mojo::JSON>, which was chosen as a model because it is
robust, minimal, and well tested. Mojo::JSON's tests were also adapted to a

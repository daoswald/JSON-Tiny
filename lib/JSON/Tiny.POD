...
distribution is excellent if you need greater control.
...
2254 lines of code spread across six modules and five files.  Tarball size is
84KB.

=item * L<JSON::Tiny>

The JSON::Tiny distribution has 351 lines of code in a single module,
and a single file.  It could even be embedded within existing code.  Tarball
size is 17KB.
...
behavioral attributes).  JSON::Tiny has four.

JSON throws exceptions if the JSON functions fail, which you will have to 
deal with.  JSON::Tiny provides an C<$jt->error> accessor instead of throwing
exceptions.
...
distribution's C<examples/> folder.  L<JSON> will automatically use L<JSON::XS>
if it's available, and in that case, JSON::XS blows all the pure-Perl modules
away.
